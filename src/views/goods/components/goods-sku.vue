  ...
  skus.forEach(sku => {
    if (sku.inventory > 0) {
      ...
      valueArrPowerSet.forEach(arr => {
        // 根据arr得到字符串 key，约定key是：['蓝色','美国'] ==> '蓝色☆美国'
        const key = arr.join(spliter)
        // console.log(key) // 给pathMap设置数据
        if (pathMap[key]) {
...
const updateDisabledStatus = (specs, pathMap) => {
  ...
  specs.forEach((item, i) => {
    ...
    item.values.forEach(val => {
      ...
      // 4. 剔除 undefined数据，按照分隔符拼接key
      const key = selectedValues.filter(value => value).join(spliter)
      // 5. 拿着key 去路径字典中查找是否有数据，有可以点击，无则禁用 (true)
...
export default {
  ...
  setup (props, { emit }) {
    ...
    const changeSku = (item, val) => {
      ...
      // 将选择的 sku信息通知父组件{skuId,price,oldPrice,inventory,specsText}
      // 1. 选择完整的 sku组合按钮，才可以拿到这些信息，提交父组件(.filter()过滤undefined)
      // 2. 不是完整的 sku组合按钮，提交空对象父组件
