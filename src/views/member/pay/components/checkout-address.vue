...
export default {
  ...
  // 1. 在拥有根元素的组建中，触发自定义使用，有没有emits选项无所谓
  // 2. 如果 组件渲染的代码片段，vue3.0规范，需要在 emits中申明所触发的自定义事件
  // 3. 提倡：所触发自定义事件，需要在emits选项申明下，代码可读性很高
  emits: ['change'],
  setup (props, { emit }) {
    // 1. 找到默认收货地址
    ...
    const successHandle = (formData) => {
      // 根据formData中的ID去当前地址列表中查找，有则修改，无则添加
      ...
      if (address) {
        ...
        // 如果是添加：往list中追加一条收货地址
        // 当修改formData时，数组中数据也会更新，因为是同一个引用地址
        // 当打开对话框需要清空之前的输入信息
        // 克隆formData数据
